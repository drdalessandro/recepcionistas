...
export const BOTS: DefBot[] = [
  ...
  { name: 'bw-cobro-programas', source: 'src/bots/cobro-programas.ts', dist: 'dist/bots/cobro-programas.js', description: 'Cron diario: renueva los programas mensuales (PB100D) cada 30 dias desde el alta.' , cron: '20 9 * * *' /* Diario, corrido de bw-cobro-membresias. Idempotente por Invoice de ciclo: correrlo de mas no cobra de mas. */ },
  ...
  { name: 'bw-web-push', source: 'src/bots/web-push.ts', dist: 'dist/bots/web-push.js', description: 'Web Push al paciente al crearse una notificación del portal (fase 4). Lo dispara una Subscription sobre Communication.' },
  ...
  { name: 'bw-purgar-auditoria', source: 'src/bots/purgar-auditoria.ts', dist: 'dist/bots/purgar-auditoria.js', description: 'Cron: borra los AuditEvent que pasaron su retención (90 días; 10 años la evidencia de una firma).' , cron: '25 * * * *' /* CADA HORA, no diario. Con el tope de 500 por corrida, una vez por día da 500 borrados contra ~1.300 eventos que cumplen plazo por día (medido 2026-09-21): la purga quedaria atras para siempre y la tabla crecería igual. Por hora son 12.000 de capacidad. Minuto 25 para no pisar a los otros crons (:00, :05, :10, :30). */ },
  ...
  { name: 'bw-teleconsulta-token', source: 'src/bots/teleconsulta-token.ts', dist: 'dist/bots/teleconsulta-token.js', description: 'Emite el JWT de acceso a la sala de Jitsi para UN turno y UNA persona, dentro de la ventana del turno. Es la puerta de la teleconsulta. Requiere JITSI_BASE_URL / JITSI_APP_ID / JITSI_JWT_SECRET.' },
  { name: 'bw-teleconsulta-presencia', source: 'src/bots/teleconsulta-presencia.ts', dist: 'dist/bots/teleconsulta-presencia.js', description: 'Registra que alguien entró a la videollamada: abre el Encounter VR, mueve el turno (arrived / checked-in) y avisa a Recepción si el paciente está esperando solo.' },
  { name: 'bw-teleconsulta-vigilante', source: 'src/bots/teleconsulta-vigilante.ts', dist: 'dist/bots/teleconsulta-vigilante.js', description: 'Cron: detecta lo que NO pasa — profesional que no se conectó, paciente que no apareció — y deja el aviso en Avisos. No cancela ni marca no-show.' , cron: '*/10 * * * *' /* Cada 10 min, el tope de sensatez del repo (tests/cron.test.ts). Alcanza porque el aviso INSTANTANEO de que el paciente entro lo da bw-teleconsulta-presencia: el vigilante solo escala y detecta ausencias, y eso tolera un tick de retraso. */ },
  { name: 'bw-estado-teleconsulta', source: 'src/bots/estado-teleconsulta.ts', dist: 'dist/bots/estado-teleconsulta.js', description: 'Solo lectura: lo operativo de una videollamada para Recepción (quién está conectado, cuánto espera, CUÁNTOS adjuntos). Nunca el contenido.' },
