...
import type { Appointment, Invoice, Patient, Resource } from '@medplum/fhirtypes';
import { EXT, ORIGENES_LEAD_LABELS, type OrigenLead } from '../fhir/identifiers.js';
import { resumenPorCanal, SIN_ORIGEN, type PacienteCanal } from '../lib/canales.js';
...
async function main(): Promise<void> {
  ...
  const filas = resumenPorCanal(pacientes, conTurno, conPago);
  ...
  console.log('Canal                     Clientes   Con turno   Con pago');
  console.log('─'.repeat(60));
  for (const f of filas) {
    console.log(
      `${etiqueta(f.origen).padEnd(26)}${String(f.clientes).padStart(6)}   ${String(f.conTurno).padStart(5)} (${pct(f.conTurno, f.clientes).padStart(4)})   ${String(f.conPago).padStart(4)} (${pct(f.conPago, f.clientes).padStart(4)})`,
    );
  }
