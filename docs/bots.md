...
| `bw-solicitar-turno` | **Portal:** crea una solicitud de turno (`Task` `code=solicitud-turno`) del paciente y avisa a Recepción por WhatsApp (`RECEPCION_WHATSAPP_TO`). No reserva: Recepción confirma. | `executeBot` desde el **portal** del paciente. |
| `bw-disponibilidad` | **Portal, SOLO LECTURA:** horarios reservables para el paciente (chips de Reservas): ventana por perfil R-13 (`tag-fm` → FM · membresía → intensidad · si no, público), capacidad R-07, desfasaje Recovery y horario del centro. Multiplaza sale como sesión grupal con `lugares`/`ocupantes`. No crea nada: la reserva sigue siendo solicitud. | `executeBot` desde el **portal** del paciente. |
