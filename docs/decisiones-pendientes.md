...
## Teleconsulta — APROBADA PARA AVANZAR (Andrés, 2026-09-16) · Fase 0 instalada
...
Jitsi con **paquetes Debian**, no Docker (comparación en su §1). El servidor
quedó **instalado y cerrado con token el 2026-09-16**, con el profesional como
único moderador; faltan cinco pruebas de aceptación y el **dominio del
Dashboard** para el `frame-ancestors`. De las nueve decisiones de
`teleconsulta.md` §10, la Fase 0 solo asume el **dominio**
...
| Consentimiento: traza de la firma (`AuditEvent` / `Provenance`) | **Pedido del portal, 2026-09-21.** Medplum tiene dos flags de servidor: `logAuditEvents` y `saveAuditEvents`. Con el segundo activado, "quién abrió el consentimiento de esta paciente, cuándo y desde qué IP" queda registrado **sin tocar una línea de código** (las lecturas se auditan: `read`, `vread`, `search`, `history`). Tres decisiones: **(a)** ¿se activa, y con qué **retención**? — audita cada lectura, el volumen y el costo no son triviales; **(b)** verificar la confianza en el proxy de nginx, o todos los eventos registran la IP del proxy y no la del paciente (esto es nuestro); **(c)** `Provenance` con `Signature` al firmar — nos inclinamos, como el portal, por que **lo escriba un bot** y no ampliar la escritura del paciente. ✅ **Versión confirmada: producción corre 5.1.39** (Andrés, 2026-09-21), que es exactamente el código del que sale el análisis: se aplica tal cual. La parte de nginx ya está hecha de nuestro lado —`deploy/nginx-api-proxy.conf` frontea toda la API y manda `X-Real-IP` / `X-Forwarded-For` en cada `location`—; falta que el **servidor** confíe en el proxy (`trust proxy`), o los eventos guardan `127.0.0.1`. Detalle en [`handoff-portal-consentimiento.md`](handoff-portal-consentimiento.md) §8.3. | ⚠️ Preguntar a Andrés |
