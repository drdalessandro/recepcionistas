...
| Consentimiento: traza de la firma (`AuditEvent`) | **RESUELTO (Andrés, 2026-09-21): se activa `saveAuditEvents`, con la purga definida desde el día uno y con la IP del cliente.** Los dos plazos quedaron en `src/lib/auditoria.ts` — **90 días** para todo, **10 años** para la escritura que crea un `Consent` o su `DocumentReference`, que es la evidencia de la firma. La purga es `bw-purgar-auditoria` (cron diario, acotada por corrida). La IP ya funciona sin tocar nada: nginx manda `X-Forwarded-For` y Medplum trae `app.set('trust proxy', 1)` hardcodeado. Activación paso a paso, prueba de humo de la IP y consultas para el día que haga falta: [`auditoria.md`](auditoria.md). | ✅ Resuelto (2026-09-21) · falta ejecutar |
| Consentimiento: `Provenance` con `Signature` | Es el recurso que FHIR tiene para firmar y hoy no existe en ningún flujo. Acordado con el portal que **lo escriba un bot** y no el paciente (no amplía la superficie de escritura de nadie, mismo criterio que `bw-ingreso-presencial`). Falta decidir **cuándo se construye**: no bloquea nada, la firma ya se prueba con el hash del adjunto contra la versión 1 y, desde hoy, con el `AuditEvent`. | A definir |
