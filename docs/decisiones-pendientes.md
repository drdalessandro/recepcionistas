...
| Consentimiento: contrato con el portal | **Parcialmente resuelto (2026-08-14)** contra un recurso real: al subir un PDF de laboratorio el portal crea un `Consent` con category ESTÁNDAR (`v3-ActCode\|IDSCL`) y el código de BW en **`policyRule`** (`procesamiento-datos-salud`) — nuestra policy y el bot ya están adaptados (la versión anterior filtraba por `category` y **habría roto la firma con 403**). Falta el **consentimiento general** del onboarding: parece registrarse como `DocumentReference`, hay que confirmar con qué `type`/`category` o pedir que además cree un `Consent` con `policyRule` = `atencion`. Ver [`handoff-portal-consentimiento.md`](handoff-portal-consentimiento.md) §2. | ⚠️ Falta el general |
| Consentimiento: alcance | ¿La señal es solo el de **Terapias Biológicas** (lo único que exige R-03 hoy) o hay un **consentimiento general de atención** que firma todo paciente? Hay códigos para los dos (`COD_CONSENTIMIENTO`), pero si el general no existe, el badge de la ficha va a decir siempre "sin firmar". | ⚠️ Confirmar con Andrés |
