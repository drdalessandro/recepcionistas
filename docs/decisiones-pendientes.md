...
| Screening: 3 preguntas sin código en la tabla | Desde 2026-08-28 el cuestionario de ingreso se EVALÚA (bug del "apto" del portal): las respuestas de riesgo pintan el banner de rojo y las mapeadas a la tabla validada bloquean/advierten por R-02. Pero **tres preguntas no tienen entrada en la tabla** y por ahora solo enrojecen el banner, sin morder en la reserva: `hbot-marcapasos` (marcapasos/implantes no certificados), `ihht-tvp` (trombosis venosa profunda activa) y `ihht-epoc` (la tabla solo tiene EPOC para HBOT, no IHHT). Candidata extra: `cirugia-reciente-ont` (cirugía de oído/nariz/tórax en los últimos 30 días) está FUERA de las secciones de screening y hoy no declara riesgo — si el DM la considera descalificante para HBOT, se suma al mapeo con su código. Agregar los códigos es decisión del Director Médico (entran como `borradorPendienteRevision` y el mapeo se completa en `SCREENING_RIESGOS`). | ⚠️ Llevar al Dr. Conrado |
