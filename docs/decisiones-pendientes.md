...
| Consentimiento: contrato con el portal | El portal no tiene definido QUÉ escribe cuando el paciente firma (la policy le habilita `QuestionnaireResponse`, `DocumentReference` y `Binary` a la vez). Se propuso el contrato `Consent` con category `CodeSystem/consentimiento` en [`handoff-portal-consentimiento.md`](handoff-portal-consentimiento.md); hasta que confirmen, la pantalla muestra "sin consentimiento firmado". | ⚠️ Esperando al portal |
