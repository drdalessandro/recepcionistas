...
| Tema | Detalle | Estado |
|---|---|---|
| Tabla de contraindicaciones | Ni v8 ni v9 la incluían; se cargó un borrador estándar HBOT/IHHT (`src/config/contraindicaciones.ts`). **Validada por el Dr. Conrado López Alonso el 2026-08-09** — pero **vuelta a BORRADOR el 2026-09-01**: el documento de admisión (Andrés → Dalessandro, 25-ago) asigna bloqueo a respuestas que la tabla tenía como advertencia, y Andrés decidió que manda el criterio más estricto. **9 entradas esperan firma médica**: 5 subidas de severidad (infección vía aérea, convulsiones, claustrofobia, embarazo, HTA no controlada — pasan de advertencia a BLOQUEO) y 4 nuevas del doc (implante no certificado, cirugía ONT reciente, EPOC severo IHHT, TVP activa). El CodeSystem está en `draft` y el seed avisa en cada corrida. **Son dos fuentes clínicas a conciliar en una sola revisión**: la de Conrado (vigente) y la que el doc pide validar a Dalessandro + Dos Santos. | ⚠️ Esperando validación médica |
